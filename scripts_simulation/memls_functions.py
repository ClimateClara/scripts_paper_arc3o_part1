...
@author: Clara Burgard, modified from A. Wiesmann, C. Mätzler, and R.T. Tonboe
"""
...
#    di = di/100. #??? needed ??? - I think yes! missing in the matlab script, maybe not actually, we have it already in cm in our files
    ...
    yh0 = np.array(freqs)
    yv0 = np.array(freqs)
    yh100 = np.array(freqs)
    yv100 = np.array(freqs)
    tb = np.array([freqs,freqs]).flatten() #To hold the tbs in runalg sequence
    emis = np.array([freqs,freqs]).flatten() #To hold the tbs in runalg sequence
    pend_h = np.array(freqs)
    pend_v = np.array(freqs)
    
    #print('set the stuff before entering the loop')
...
#          print 'di', di
          ...
          #print('calculating the epsi1')

          #epsi and epsii if wet
          [epsi,epsii] = mixmod(freq,Ti,Wi,epsi,epsii)
          
          #print('calculating the epsi2')
          ...
          #print('calculating the epsi3')

          [epsi,epsii] = mysie(my,roi,Ti,sal,freq,epsi,epsii)
          
          #print('calculating the epsis')
          ...
          ##### coherent layer stuff commented on 03.04.2018 after email exchange with Rasmus and Mai
...
#          ###
          ...
          yh[ifreq] = round(Tbh,2)
          tb[2*ifreq+1] = round(Tbh,2)
          ...
          yv[ifreq] = round(Tbv,2)
          tb[2*ifreq] = round(Tbv,2)
    ...
    #print('finished emissivities')

    
    return x, eh, ev, yh, yv, Teffh, Teffv, pend_h, pend_v
