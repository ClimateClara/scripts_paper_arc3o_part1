  ...
  Subroutine  sub_heat_fluxes()
    ...
    REAL(wp) ::T_old, k_1, T_b,emi, pen, temp, temp1, temp2
       ...
       IF (boundflux_flag==2) THEN
          ...
          temp2 = (1._wp-albedo)*fl_sw+fl_rest+3._wp*sigma*T_old**4._wp-temp1*4.*sigma*T_old**3._wp
          temp1 = temp1-zeroK
       ...
       IF (abs((temp1-temp2)/dt) > 0.00001_wp) then
          print*,'energie discrepencies during heat flux calculations higher than 0.00001 Jounles per second',(temp1-temp2)/dt
          STOP 431
       end if
