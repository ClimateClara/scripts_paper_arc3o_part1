...
CONTAINS
  ...
  SUBROUTINE output_settings(description,testcase,N_top,N_bottom,Nlayer,fl_q_bottom,T_bottom,S_bu_bottom,thick_0,time_out,    &
    ...
    REAL(wp),        INTENT(in) :: fl_q_bottom,T_bottom,S_bu_bottom,thick_0,time_out,time_total,dt 
  ...
  SUBROUTINE output(Nlayer,T,psi_s,psi_l,thick,S_bu,ray,format_T,format_psi, &
    ...
    REAL(wp), DIMENSION(Nlayer),   INTENT(in) :: T,psi_s,psi_l,thick,S_bu
    ...
    CHARACTER*12000,               INTENT(in) :: format_T,format_psi,format_thick,format_snow
  ...
  SUBROUTINE output_begin(Nlayer,debug_flag,format_T,format_psi,format_thick,format_snow,format_T2m_top)
    ...
    CHARACTER*12000, INTENT(out) :: format_T,format_psi,format_thick,format_snow,format_T2m_top
    INTEGER                      :: k
  ...
  SUBROUTINE output_begin_bgc(Nlayer,N_bgc,format_bgc)
    ...
    DO k=1,N_bgc
       IF (k<10) THEN
          WRITE(output_string,'(A18,I1,A7)')'./output/dat_bgc0',k,'.bu.txt'
          OPEN(2*k+400,file=TRIM(output_string),STATUS='replace',Recl=12288)
          WRITE(output_string,'(A18,I1,A7)')'./output/dat_bgc0',k,'.br.txt'
          OPEN(2*k+401,file=TRIM(output_string),STATUS='replace',Recl=12288)
       ELSE 
